...
- **Redundant Configs:** Do not parse the `Pipfile` (rely on `requirements.txt` or `pyproject.toml` instead).
- **Active Secrets:** Never look inside `.env`, `.env.*`, `config/env/.env`, or `config/env/*.local`. (You may look at `.env.example` templates to view required variable keys).
- **Admin Overhead:** Skip all community, templates, and text files inside `.github/` EXCEPT for `.github/workflows/` which must remain accessible for CI/CD pipeline tasks.
- **Build & Runtime Artifacts:** Skip all Python runtime caches (`__pycache__/`, `*.pyc`, `*.pyo`, `*.pyd`), test outputs (`.pytest_cache/`, `htmlcov/`), and build artifacts (`dist/`, `build/`, `*.egg-info/`).
